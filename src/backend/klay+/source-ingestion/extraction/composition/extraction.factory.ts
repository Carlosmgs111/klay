...
import type { ExtractionInfrastructurePolicy, ResolvedExtractionInfra } from "./infra-policies";
import type { ExtractionUseCases } from "../application/index";
...
export async function extractionFactory(
  ...
  const { ExtractionComposer } = await import("./ExtractionComposer");
  const infra = await ExtractionComposer.resolve(policy);

  // 2. Construct use cases with resolved dependencies
  const { ExtractionUseCases } = await import("../application/index");
