...
import type { SourceInfrastructurePolicy, ResolvedSourceInfra } from "./infra-policies";
import type { SourceUseCases } from "../application/index";
...
export async function sourceFactory(
  ...
  const { SourceComposer } = await import("./SourceComposer");
  const infra = await SourceComposer.resolve(policy);

  // 2. Construct use cases with resolved dependencies
  const { SourceUseCases } = await import("../application/index");
